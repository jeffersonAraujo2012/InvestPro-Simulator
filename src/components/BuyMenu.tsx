...
interface IHideBuyMenu {
  hideBuyMenu: () => void;
}

export default function BuyMenu({hideBuyMenu}: IHideBuyMenu) {
  ...
  return (
    <BlackFullScreen>
      <div className="relative bg-white rounded-[8px] w-5/12 p-12">
        <AiOutlineClose className="absolute top-8 right-8 w-8 h-8 fill-[#1C1F37] cursor-pointer" onClick={() => {
          hideBuyMenu();
        }}/>
        ...
        <div className="font-['Manrope'] [&>p]:mb-1 [&>p]:text-xl [&>p]:flex [&>p]:justify-between mb-4">
          ...
          <p>
            <span className="font-bold">Saldo após a compra:</span>
            <span>
              {"R$ " +
                (
                  balance -
        ...
        <ButtonStyled fullWidth>Comprar</ButtonStyled>
      </div>
    </BlackFullScreen>
  );
}
